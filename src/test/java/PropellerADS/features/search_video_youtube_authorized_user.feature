@search_video_youtube_authorized_user
@test

Feature: validation request for authorized user
    ...
    Scenario: Auth user
        ...
        When set email/phone in field login "burylov.artem.perm"
        When press button further
        When in input password "password" set text "укажите пароль"
        When waiting
        When press button further
        When waiting
        When check users auth "burylov.artem.perm"
